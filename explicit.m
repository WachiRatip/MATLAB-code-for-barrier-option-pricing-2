...
%
% explicit(spot,strike,barrier,term,r,d,vola_alpha,N,M)
%
...
function [call, V] = explicit(stock,strike,barrier,term,r,d,vola_alpha,N,M)
...
% Solve BS PDE by using the explicit scheme
V = zeros(N-1,M+1);
...
for k=1:M
    % Construct AE
    % note that: For the stability of explicit scheme;
    % absolute values of all eigenvalues of AE must be less than 1
    AE = diag(d(:,k)) + diag(u(1:N-2,k),1) + diag(l(2:N-1,k),-1);
